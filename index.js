 ...
 *   - POSTs to Zapier OAuth Catch Hook (for Google Sheets logging)
 */
...
const {
  ...
  // Zapier Catch Hook URL for OAuth logging (Google Sheets member log)
  ZAPIER_OAUTH_HOOK_URL,
 ...
 * Matches by keywords in tier name, so renaming like "Journeyman 15% discount"
 * still matches.
 */
...
/**
 * POST to Zapier Catch Hook for OAuth logging (Google Sheets member log).
 * IMPORTANT: This is what makes your "Ghost OAuth Zap" fire live.
 */
async function postToZapierOAuth(payload) {
  if (!ZAPIER_OAUTH_HOOK_URL) {
    console.warn("ZAPIER_OAUTH_HOOK_URL not set; skipping OAuth log");
    return;
  }

  const res = await fetch(ZAPIER_OAUTH_HOOK_URL, {
    method: "POST",
  ...
  if (!res.ok) {
    ...
    throw new Error(`Zapier OAuth hook failed: ${res.status} ${txt}`);
  }
}
...
app.post("/ghost/resolve-tier", async (req, res) => {
  try {
    ...
    const labelsSig = member
      ? getLabelsSignature(member, labelsFromZap)
      : labelsFromZap || "none";
    ...
    return ok(res, {
      ...
      tier_sig: tierName,
      label_sig: labelsSig,
      storage_key, // <- Zapier Storage step MUST use this as Key
      discord_link, // <- email body links to this
    });
  } catch (err) {
    return ok(res, {
      ok: false,
...
app.get("/discord/callback", async (req, res) => {
  try {
    ...
    if (!joinRes.ok) {
      ...
      return res
        .status(400)
        .send(`Failed to join guild. Discord said ${joinRes.status}: ${txt}`);
    }
    ...
    // ✅ NEW: Fire Zapier OAuth logging webhook (for Google Sheets logging)
    // This is what makes Zap #1 Ghost OAuth Zap actually run live.
    await postToZapierOAuth({
